<?php

namespace App\Docs\Operations\Collections\Snomed;

use App\Docs\Responses\ResourceDeletedResponse;
use App\Docs\Tags\CollectionSnomedTag;
use GoldSpecDigital\ObjectOrientedOAS\Objects\BaseObject;
use GoldSpecDigital\ObjectOrientedOAS\Objects\Operation;

class DestroyCollectionSnomedOperation extends Operation
{
    /**
     * @param string|null $objectId
     * @throws \GoldSpecDigital\ObjectOrientedOAS\Exceptions\InvalidArgumentException
     * @return static
     */
    public static function create(string $objectId = null): BaseObject
    {
        return parent::create($objectId)
            ->action(static::ACTION_DELETE)
            ->tags(CollectionSnomedTag::create())
            ->summary('Delete a specific SNOMED collection')
            ->description('**Permission:** `Super Admin`')
            ->responses(
                ResourceDeletedResponse::create(null, 'collection SNOMED')
            );
    }
}
